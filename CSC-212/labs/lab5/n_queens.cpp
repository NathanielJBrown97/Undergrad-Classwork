...
bool NQueens::solve_rec(int col){
    ...
    for (unsigned int cur_row = 0; cur_row < this->board_size; cur_row++){
        //check if valid pos
        if (is_valid_row(cur_row,col)){
            // queen at this iteration of position
            (*this->board)[cur_row][col]=1;
            //recursive call next move to next column
            if (solve_rec(col+1)){
                return true; // if col+1 is solution, return true
            }
            //otherwise --- remove queen from this iteration of position
            (*this->board)[cur_row][col] = 0;
        }
    }
    return false;
